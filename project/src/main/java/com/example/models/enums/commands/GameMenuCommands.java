...
public enum GameMenuCommands implements Command {
    ...
    GAME_MAP("^game\\s+map\\s+(?<map_number>\\d+)$"),
    ...
    CRAFTING_CRAFT("^crafting\\s+craft\\s+(?<item_name>.+)$"),
    PLACE_ITEM("^place\\s+item\\s+-n\\s+(?<item_name>.+?)\\s+-d\\s+(?<direction>.+)$"),
    CHEAT_ADD_ITEM("^cheat\\s+add\\s+item\\s+-n\\s+(?<item_name>.+?)\\s+-c\\s+(?<count>\\d+)$"),
    ...
    //TODO: Note from Amin to Soroosh: Everything above this comment has been implemented. Implement 52 to 94
    ...
    ARTISAN_USE("^artisan\\s+use\\s+(?<artisan_name>.+?)\\s+(?<item1_name>.+)$"),
    ARTISAN_GET("^artisan\\s+get\\s+(?<artisan_name>.+)$"),
    SHOW_ALL_PRODUCTS("^show\\s+all\\s+products$"),
    SHOW_ALL_AVAILABLE_PRODUCTS("^show\\s+all\\s+available\\s+products$"),
    PURCHASE("purchase\\s+(?<product_name>.+?)\\s+-n\\s+(?<count>\\d+)"),
    CHEAT_ADD_DOLLARS("^cheat\\s+add\\s+(?<count>\\d+)\\s+dollars$"),
    SELL_PRODUCT("^sell\\s+(?<product_name>.+?)\\s+-n\\s+(?<count>\\d+)$"),
    ...
    GIFT_RATE("^gift\\s+rate\\s+-i\\s+(?<gift_number>\\d+)\\s+-r\\s+(?<rate>\\d+)$"),
    ...
    MEET_NPC("^meet\\s+NPC\\s+(?<npc_name>.+)$"),
    GIFT_NPC("^gift\\s+NPC\\s+(?<npc_name>.+?)\\s+-i\\s+(?<item>.+)$"),
