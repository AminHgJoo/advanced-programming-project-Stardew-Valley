...
public class CookingMenu implements MyScreen {
    ...
    private CookingRecipes selectedRecipe;

    public CookingMenu(GameMain gameMain, FarmMenu farmMenu) {
    ...
    private void initializeStage() {
        ...
        for (int row = 0; row < descriptions.length; row++) {
            for (int col = 0; col < descriptions[row].length; col++) {
                ...
                CookingRecipes recipes = descriptions[row][col];
                ...
                if (!player.getUnlockedCookingRecipes().contains(recipes)) {
        ...
        TextButton craftButton = new TextButton("Cook", skin);
    ...
    private String localController(CookingRecipes targetRecipe) {
        ...
        for (Slot ingredient : targetRecipe.ingredients) {
            ...
            if (playerSlot == null) {
                return "You don't have the required ingredients to craft this item!";
            }
