...
public class ShippingMenu implements MyScreen, InputProcessor {
    private final GameMain gameMain;
    private final int ITEMS_PER_PAGE = 27;
    private final int shippingScale = 4;
    ...
    public boolean keyDown(int i) {
        if (i == Input.Keys.ESCAPE || i == Input.Keys.J) {
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        for (int i = 0; i < ITEMS_PER_PAGE; i++) {
            ...
            Texture shippingTexture = AssetManager.getImage("shippingbin");
            int shippingWidth = shippingTexture.getWidth() * shippingScale;
            int shippingHeight = shippingTexture.getHeight() * shippingScale;
            int shippingX = Gdx.graphics.getWidth() - shippingWidth;
            int shippingY = Gdx.graphics.getHeight() / 2 - shippingHeight / 2;

            if (screenX >= shippingX && screenX <= shippingX + shippingWidth &&
                screenY >= shippingY && screenY <= shippingY + shippingHeight) {
                if (selectedSave >= 0) {
    ...
    public void render(float delta) {
        ...
        Texture shippingTexture = AssetManager.getImage("shippingbin");
        batch.draw(shippingTexture, Gdx.graphics.getWidth() - shippingTexture.getWidth() * shippingScale, Gdx.graphics.getHeight() / 2 - shippingTexture.getHeight() * shippingScale / 2, shippingTexture.getWidth() * shippingScale, shippingTexture.getHeight() * shippingScale);
